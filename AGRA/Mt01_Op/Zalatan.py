...
def generarPosibilidades(numero):
    """
    Genera y retorna una lista de los posibles movimientes a partir de un numero.
    ...
    for indice in range(4):
        ...
        listaPosibilidades.append(int(newStr))
...
def bfs(inicio, target, listaForbiddenn):
    ...
    if inicio == target:
        ...
        while len(cola) > 0 and not encontrado:
            ...
            posibilidadesList = generarPosibilidades(posicionActual)
            indice = 0
            while indice < len(posibilidadesList) and not encontrado:
                posibilidad = posibilidadesList[indice]
