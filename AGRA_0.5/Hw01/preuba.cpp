...
void procesarJuego(vector<int> entrada) {
    Estado estado;
    estado.baraja.assign(entrada.begin(), entrada.end());
    ...
    while (!juegoTerminado) {
        ...
        // Continuar el juego si no se ha terminado
            ...
            estado.baraja.pop_front();
            estado.pilas[pilaSiguiente].push_back(carta);
            ++movimientos;
            ...
            while (cambio) {
                cambio = false;
                vector<int> combinacion = buscarCombinaciones(estado.pilas[pilaSiguiente]);
