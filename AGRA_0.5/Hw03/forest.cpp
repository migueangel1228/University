...
Ultimo cambio agragar complejiad y intentar colocar mas bonito

Complejidad: La complejidad de este programa depende de dos partes. L

La primera trata de el recorrido DFS, la cual su complejidad tempora es  O(n*m) donde n son las N filas de la matriz en cada caso y M las columnas.

La segunda es el ordenamiento, que se ahce al final del programa para generar una solucion correcta, se ordenan K elemenots que orresponde a los Groves,
pero en el peor de los caso habran N*M groves si  todos los arboles  adyacentes unos a otrso son de diferentes especie. Por las carateristicas de c++,
es O((n*m) * (Log(n*m))) donde n = N y m = M.

finalmete concluimos que la complejidad temporal en general por cada caso es: O( n*m + ((n*m) * (Log(n*m))) )

*/

// Función DFS para explorar un groves de trees
pair<string, int> explorarGrove(int filaInicio, int columnaInicio, 
                               vector<vector<pair<string, int> > >& matrizEspeciesAlturas,
                               vector<vector<bool> >& visitados,
                               int totalFilas, int totalColumnas) {
    
    string especieGrove = matrizEspeciesAlturas[filaInicio][columnaInicio].first;
    int maxHeight = matrizEspeciesAlturas[filaInicio][columnaInicio].second;
    
    stack<pair<int, int>> Stack;
    Stack.push(make_pair(filaInicio, columnaInicio));
    ...
    while (!Stack.empty()) {
        ...
        int filaActual = treeActual.first;
        int columnaActual = treeActual.second;

        // Explorar todas las direcciones posibles (horizontales y verticales)
        for (int d = 0; d < 4; ++d) {
            int newRow = filaActual + direcciones[d].first;
            int newCol = columnaActual + direcciones[d].second;

            // Verificar todos los limites de la matriz (direcciones)
            if (newRow >= 0 && newRow < totalFilas && 
                newCol >= 0 && newCol < totalColumnas) {
    ...
    for (int casoActual = 1; casoActual <= totalCases; ++casoActual) {
        ...
        for (int i = 0; i < filas; ++i) {
            for (int j = 0; j < columnas; ++j) {
                ...
                int tamanoEntradaInt = entrada.size();
                while (k < tamanoEntradaInt && entrada[k] != '#') {
                    especie += entrada[k];
                    k++;
                }
                // Saltar el '#'
        ...
        // Printear resultados
        int tamanoResultados =  resultados.size();
        cout << "Forest #" << casoActual << "\n";
        for (int indice = 0; indice < tamanoResultados; ++indice) {
