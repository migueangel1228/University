...
Problem D - Lift Hopping in the Real World
"""
...
def peorCaso(weight, pisos, floor):
    primerPiso = pisos[0]
    ultimoPiso = pisos[-1]

    distanciaAlPrimerPiso = abs(primerPiso - floor)
    distanciaAlUltimoPiso = abs(ultimoPiso - floor)

    mayorDistancia = max(distanciaAlPrimerPiso, distanciaAlUltimoPiso)
    tiempoPeorCaso = mayorDistancia * weight

    return tiempoPeorCaso
...
def dijkstra(elevatorWeights, matrizPisos, k):
    ...
    for e in range(n):
        if matrizPisos[e][0] == 0:
            wait = peorCaso(elevatorWeights[e], matrizPisos[e], 0)
            dist[0][e] = wait
            heappush(pq,(wait, 0, e)) # time actual(peso), floor, elevator
        ...
        if dist[floor][elev] == time:
            ...
            for e2 in range(n):
                if e2 != elev and floor in matrizPisos[e2]:
                    wait = peorCaso(elevatorWeights[e2], matrizPisos[e2], floor)
                    newTime = time + 5 + wait
